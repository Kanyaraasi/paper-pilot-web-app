...
const TestCreationDashboard = ({ onNavigateToDashboard }) => {
  const [subjectError, setSubjectError] = useState('');
  const { setCurrentStep, markStepCompleted, markStepIncomplete } = useProgress();
  ...
  // Watch form fields to determine if step should be marked as completed
  const watchedFields = watch();
  
  useEffect(() => {
    const requiredFields = ['name', 'duration', 'totalMarks', 'class', 'unit', 'academicYear', 'batch'];
    const hasSelectedSubject = subjects.some(subject => subject.selected);
    const allRequiredFieldsFilled = requiredFields.every(field => watchedFields[field] && watchedFields[field].trim() !== '');
    
    if (allRequiredFieldsFilled && hasSelectedSubject && Object.keys(errors).length === 0) {
      markStepCompleted(1);
  ...
  }, [watchedFields, subjects, errors, markStepCompleted, markStepIncomplete, setCurrentStep, onNavigateToDashboard]);
